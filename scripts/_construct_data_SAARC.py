...
# Data source 
URL_ = "http://www.saarc-sec.org/Public-Service-Commissions-of-SAARC-Member-States/110/"
...
# Xpath extraction
_xpath='//*[@id="Header"]/div/div[2]/ul/li/span/text()'
...
def url_local_request (url):
    ...
    o = urlparse(url)
    path = o.path
    host, fn_local = os.path.split(path)
    if fn_local == "":
        fn_local = path.strip('/').split('/')[-1]
        print ([path, fn_local])
    fn_local = os.path.join(path_data, fn_local)
    
    print ([host, fn_local])  #debug
    try:
        tree = parse(fn_local)
    except:
        r = url_request (url) 
